...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ['input-cpf', 'input-busca-cpf'].forEach((id) => document.getElementById(id)?.addEventListener('input', (event) => { event.target.value = formatarCPF(event.target.value); }));
});
...
async function carregarProfissionais(especialidade = '', nome = '') {
  ...
  try {
    ...
    const res = await fetch(API_URL + '/profissionais?' + params.toString());
    if (!res.ok) throw new Error();
    profissionaisCache = await res.json();
    if (container) container.innerHTML = profissionaisCache.length ? profissionaisCache.map((p) => '<div class="card-profissional"><div class="avatar"><img src="' + p.foto + '" alt="' + p.nome + '" onerror="this.onerror=null;this.style.display=\'none\';this.nextElementSibling.style.display=\'flex\';"><div class="avatar-fallback">' + obterIniciais(p.nome) + '</div></div><h3>' + p.nome + '</h3><span class="badge">' + p.especialidade + '</span><p class="registro"><strong>Registro:</strong> ' + p.registro + '</p><p class="bio">' + p.bio + '</p></div>').join('') : '<p class="alerta">Nenhum profissional encontrado com os filtros informados.</p>';
...
function filtrarProfissionais(event) {
  ...
  carregarProfissionais(document.getElementById('filtro-especialidade')?.value.trim() || '', document.getElementById('filtro-nome')?.value.trim() || '');
}

function carregarProfissionaisDoAgendamento() {
  const especialidade = document.getElementById('select-especialidade').value;
  ...
  selectData.innerHTML = '<option value="">-- Selecione o profissional primeiro --</option>';
  selectHorario.innerHTML = '<option value="">-- Selecione a data primeiro --</option>';
  selectProfissional.disabled = true; selectData.disabled = true; selectHorario.disabled = true;
  if (!especialidade) return;
  const profissionais = profissionaisCache.filter((p) => p.especialidade === especialidade);
  selectProfissional.innerHTML += profissionais.map((p) => '<option value="' + p.id + '">' + p.nome + '</option>').join('');
  selectProfissional.disabled = profissionais.length === 0;
}

function atualizarDatas() {
  const id = Number(document.getElementById('select-profissional').value);
  const selectData = document.getElementById('select-data');
  const selectHorario = document.getElementById('select-horario');
  selectData.innerHTML = '<option value="">-- Selecione a data --</option>';
  selectHorario.innerHTML = '<option value="">-- Selecione a data primeiro --</option>';
  selectData.disabled = true; selectHorario.disabled = true;
  const profissional = profissionaisCache.find((p) => p.id === id);
  if (!profissional) return;
  profissional.dias_disponiveis.forEach((data) => {
    const partes = data.split('-');
    const option = document.createElement('option'); option.value = data; option.textContent = partes[2] + '/' + partes[1] + '/' + partes[0];
...
async function atualizarHorarios() {
  const id = Number(document.getElementById('select-profissional').value);
  const data = document.getElementById('select-data').value;
  const selectHorario = document.getElementById('select-horario');
  selectHorario.innerHTML = '<option value="">Carregando horários...</option>'; selectHorario.disabled = true;
  if (!id || !data) return;
  try {
    const res = await fetch(API_URL + '/disponibilidade?profissional_id=' + id + '&data=' + encodeURIComponent(data));
    if (!res.ok) throw new Error();
    const resultado = await res.json();
    selectHorario.innerHTML = '<option value="">-- Selecione o horário --</option>' + resultado.horarios.map((h) => '<option value="' + h + '">' + h + '</option>').join('');
    if (!resultado.horarios.length) { selectHorario.innerHTML = '<option value="">Nenhum horário disponível</option>'; return; }
...
async function criarAgendamento(event) {
  event.preventDefault();
  const msgBox = document.getElementById('mensagem-agendamento'); msgBox.className = 'mensagem'; msgBox.textContent = 'Processando agendamento...';
  ...
  try {
    const res = await fetch(API_URL + '/agendamentos', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    ...
    msgBox.className = 'mensagem sucesso'; msgBox.textContent = '✅ ' + data.mensagem;
    document.getElementById('form-agendamento').reset();
    carregarProfissionaisDoAgendamento();
...
async function buscarConsultasPorCPF(event) {
  event.preventDefault();
  const cpf = obterCPFNumerico(document.getElementById('input-busca-cpf').value);
  const container = document.getElementById('resultado-consultas'); container.innerHTML = '<p>Buscando agendamentos...</p>';
  try {
    const res = await fetch(API_URL + '/agendamentos/' + cpf); const data = await res.json();
...
async function cancelarAgendamento(idAgendamento) {
  ...
  try {
    const res = await fetch(API_URL + '/agendamentos/' + idAgendamento, { method: 'DELETE' }); const data = await res.json();
